...
export const WellnessPage: React.FC = () => {
  ...
  // FIX: Added familyMemberId to the state, defaulting to the current user.
  ...
  const addWellnessEntry = () => {
    ...
    setCurrentEntry({ date: new Date().toISOString().split('T')[0], familyMemberId: currentUser.id }); // Reset form
  };
        ...
        <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                    ...
                    <Select id="familyMemberId" name="familyMemberId" options={familyMemberOptions} value={currentEntry.familyMemberId || ''} onChange={handleInputChange} />
                </div>
                <div className="space-y-2">
        ...
        <CardContent>
            ...
            <ul className="space-y-4 max-h-96 overflow-y-auto">
                ...
                <li key={entry.id} className="p-4 bg-surface-soft rounded-lg border border-border">
                    ...
                    <p className="text-sm font-semibold text-textSecondary">For: {familyMembers.find(fm => fm.id === entry.familyMemberId)?.name || 'Unknown'}</p>
