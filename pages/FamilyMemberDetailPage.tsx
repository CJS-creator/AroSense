...
import { TimelineEvent, VitalSign, VaccinationRecord } from '../types';
...
export const FamilyMemberDetailPage: React.FC = () => {
  ...
  const { documents, prescriptions, vitals, setVitals, vaccinations, setVaccinations, appointments } = useHealthRecords();
  ...
  // FIX: Explicitly typed the return of each .map() to TimelineEvent to ensure type consistency.
  const timelineEvents = useMemo<TimelineEvent[]>(() => {
    ...
    const memberVitals = vitals.filter(v => v.familyMemberId === memberId).map((v): TimelineEvent => {
        const details = [
            ...
            v.bloodPressure ? `${v.bloodPressure} BP` : null,
            v.heartRate ? `${v.heartRate}bpm` : null
        ].filter(Boolean).join(', ');
        return { 
            id: v.id, 
            type: 'vital', 
            title: `Vitals Logged: ${details || 'Entry created'}`, 
            date: v.date 
        };
    });
    ...
    const memberWellness = wellnessLog.filter(w => w.familyMemberId === memberId).map((w): TimelineEvent => {
        const details = [
            w.mood,
            w.sleepHours ? `${w.sleepHours}h sleep` : null,
            w.waterIntakeLiters ? `${w.waterIntakeLiters}L water` : null,
            w.activity || null,
        ].filter(Boolean).join(' • ');
    ...
    return [...memberDocuments, ...memberPrescriptions, ...memberNotes, ...memberBills, ...memberVitals, ...memberVaccinations, ...memberAppointments, ...memberWellness]
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

  }, [memberId, documents, prescriptions, medicalNotes, bills, insurancePolicies, vitals, vaccinations, appointments, wellnessLog]);
  ...
  const handleAddVital = (newVital: Omit<VitalSign, 'id' | 'familyMemberId'>) => {
    const vitalToAdd: VitalSign = { ...newVital, id: `vtl-${Date.now()}`, familyMemberId: member.id };
    setVitals(prev => [...prev, vitalToAdd]);
  };

  const handleAddVaccination = (newVaccination: Omit<VaccinationRecord, 'id' | 'familyMemberId'>) => {
    const vaccinationToAdd: VaccinationRecord = { ...newVaccination, id: `vac-${Date.now()}`, familyMemberId: member.id };
    setVaccinations(prev => [...prev, vaccinationToAdd]);
  };
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2">
              <Card className="h-full">
                  ...
                  <CardContent className="max-h-[500px] overflow-y-auto"><Timeline events={timelineEvents} /></CardContent>
              </Card>
          </div>
        </div>
      ...
      <AddVitalModal isOpen={isVitalModalOpen} onClose={() => setIsVitalModalOpen(false)} onAddVital={handleAddVital} />
      <AddVaccinationModal isOpen={isVaccinationModalOpen} onClose={() => setIsVaccinationModalOpen(false)} onAddVaccination={handleAddVaccination} />
    </>
  );
};